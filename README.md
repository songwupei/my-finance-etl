# 集团财务数据 AI 智能分析与可视化平台

基于 Kedro 数据管道框架的 AI 驱动财务数据 ETL 与可视化平台，集成大模型（LLM）自然语言智能查询，支持动态 Excel 文件扫描、指标标准化处理、组织树构建、资金账户解析、地理编码与可视化展示。**Vizro + Shiny + MapLibre 三引擎架构** — Vizro 面向领导汇报大屏，Shiny 面向个人电脑办公大屏，MapLibre 面向企业地图可视化。
...
| 能力 | 说明 |
|---|---|
| **自然语言 → SQL** | 基于 LLM（Anthropic Claude）的自然语言转 SQL 引擎，用户用中文描述需求即可自动生成 DuckDB 查询 |
| **KPI 联动** | 查询结果自动联动账户总数、余额合计、银行类型分布等关键财务指标卡片 |
| **地理分布图** | 自动渲染国内城市 + 海外国家地理分布气泡图，颜色按银行类型（合作行/非合作行/财务公司）区分 |
| **多维细分表** | `great_tables` 精美格式化表格，支持按子集团/银行/省份/城市多维度下钻，含财务快报数据（财务公司存款 + 商业银行存款） |
| **交互式明细** | 可排序、可筛选的交互式数据表，支持 CSV 一键导出 |
| **零代码探索** | 决策者无需 SQL 技能即可自助探索数据，秒级响应 |
| **双模式查询** | QueryChat 通用问答 + 预置快捷查询（按子集团/按银行/按地区等常见分析场景） |
...
- **Shiny FinForge 布局升级** 🎨: 导航栏对齐 FinForge 风格 — emoji 图标、双层导航、面板重命名，移除占位页面
- **全屏地图** 🗺️: 资产地图、资金地图全屏铺满 (`100vh - 58px`)，Plotly autosize 自适应
- **暗黑模式支持** 🌙: 支持 `ui.input_dark_mode()` Bootstrap 原生明暗切换
- **余额统计优化** 📊: 整体情况 KPI 卡片增加日期标注，图表全宽填充 (`fill=True`)
- **穿透监管重构** 🔍: PanReg 页面按日期扫描报告文件，支持文件多选浏览
- **代码简化**: 移除重复代码，统一 `page()`/`server()` 签名，净减 ~200 行

### v1.6.7 (2026-07-14)

- **日报生成修复** 🔧: Quarto 环境安装 `papermill` 包 — 修复 `-P` 参数注入失败导致日报渲染报错 (`--execute-params` 依赖 papermill)
- **日报发送修复** 📧: `mailops` 统一在 `quarto` 环境安装并复制 `accounts.yml` 配置 — 修复邮件发送命令找不到和配置文件路径解析错误
- **排错笔记** 📝: 新增 `doc/troubleshooting-notes.md` — 收录 OOM、DuckDB NaN、Kedro 执行顺序、日报生成/发送等 7 条经验

### v1.6.6 (2026-07-14)

- **README 重构** 📝: 标题升级为「AI 智能分析与可视化平台」、新增 AI 智能查询专区（7 项核心能力）、参赛定位标注（集团公司 2026 数智化场景创新应用大赛 · 人工智能赛道）
